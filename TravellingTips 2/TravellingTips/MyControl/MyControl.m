...
+(UIButton *)creatButtonWithFrame:(CGRect)frame target:(id)target sel:(SEL)sel tag:(NSInteger)tag image:(NSString *)name selectImage:(NSString *)selectName title:(NSString *)title{
    UIButton *button = nil;
    if (name) {
        //创建图片按钮
        //创建背景图片 按钮
...
+(UISearchBar *)createSearchBarWithFrame:(CGRect)frame placeHolder:(NSString *)placeHolder delegate:(id<UISearchBarDelegate>)delegate{
